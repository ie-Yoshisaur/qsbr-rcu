...
impl<T> Rcu<T> {
    ...
    fn rcu_read_lock(&self) {
        THREAD_LOCAL_STRAGE.with(|tls_head| {
    ...
    pub fn register_thread(&self) {
        THREAD_LOCAL_STRAGE.with(|tls_head| {
            ...
            let new_tls = Box::new(ThreadLocalStrage::new(self.rcu_id, new_thread_data_ptr));
...
thread_local! {
    /// Thread-local storage for each thread's `ThreadLocalStrage` linked list.
    ///
    /// Each thread maintains a linked list of `ThreadLocalStrage` instances, one for each RCU instance
    /// it participates in. This structure is essential for implementing multiple RCU instances per thread.
    static THREAD_LOCAL_STRAGE: AtomicPtr<ThreadLocalStrage> = AtomicPtr::new(ptr::null_mut());
}

/// Structure representing thread-local storage for an RCU instance.
///
/// Each `ThreadLocalStrage` contains its own `ThreadData`, a unique `rcu_id`, and a pointer to the next
/// `ThreadLocalStrage` in the thread's linked list.
pub struct ThreadLocalStrage {
    thread_data: *mut ThreadData,
    rcu_id: usize,
    next: AtomicPtr<ThreadLocalStrage>,
}

impl ThreadLocalStrage {
    /// Creates a new `ThreadLocalStrage` with the specified RCU ID and ThreadData pointer.
    ///
    /// # Parameters
    ///
...
/// Retrieves the current thread's `ThreadLocalStrage`.
///
/// This function provides access to the thread-local `ThreadLocalStrage` linked list, which is
/// used internally by the RCU implementation to track the thread's state for multiple RCU instances.
pub fn get_current_thread_strage() -> *mut ThreadLocalStrage {
    THREAD_LOCAL_STRAGE.with(|tls_head| tls_head.load(Ordering::Acquire))
}
...
/// `ThreadLocalStrage` with the provided `Rcu` instance.
///
/// This macro ensures that the thread participates in RCU operations by registering its
/// `ThreadLocalStrage` upon creation and unregistering it before termination.
#[macro_export]
